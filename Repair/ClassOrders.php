...
$sql="SELECT * FROM repairs WHERE create_user_id=? ";

if($isClassTch=="1"){
  $sql.="OR create_user_id IN (SELECT UserID FROM users WHERE SchoolGrade='{$schoolGrade}' AND SchoolClass='{$schoolClass}') ";
}

$sql.="ORDER BY status DESC,create_time";
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <table class="table table-hover table-striped table-bordered" style="border-radius: 5px; border-collapse: separate;">
          ...
          <tr>
            <th>报修发起人</th>
            <td><p id="realName" style="color:blue"></p></td>
          </tr>
          <tr>
            <th>设备所在场室</th>
