...
/**
* -------------------------------------
* checkAJAXToken 检测AJAX的Token的有效性
* -------------------------------------
* @param String 待检测的Token值
* -------------------------------------
*/
function checkAJAXToken($token)
{
  $timestamp=getSess(Prefix."AJAX_timestamp");
  $trueToken=sha1(session_id().md5($timestamp));
  
  if($token==$trueToken) return "1";
  else return "0";
}
